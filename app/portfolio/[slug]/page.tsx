...
const PORTFOLIO_DETAIL_COLUMNS =
  "title,summary,outcomes,cover_url,gallery_urls,video_urls,case_study_slug,slug";
const BASE_PORTFOLIO_DETAIL_COLUMNS =
...
async function getPublishedPortfolioItem(rawSlug: string) {
  ...
  if (exactError?.message.includes("gallery_urls")) {
    const fallback = await supabasePublic
      .from("portfolio")
      .select(BASE_PORTFOLIO_DETAIL_COLUMNS)
      .eq("slug", slug)
      .eq("status", "published")
      .limit(1)
      .maybeSingle();
